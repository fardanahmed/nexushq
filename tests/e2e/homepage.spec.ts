...
test.describe('Homepage E2E', () => {
  test('should load without errors and render main sections', async ({ page }) => {
    ...
    await expect(page.locator('h1').filter({ hasText: 'Masterclass Experience' })).toBeVisible();

    // 4. Verify TrustBar is visible (use first to avoid strict mode error due to marquee loop)
    await expect(page.locator('span.font-heading').filter({ hasText: 'Stripe' }).first()).toBeVisible();

    // 5. Verify the new Bento Box Features section
    await expect(page.locator('h2').filter({ hasText: /^Everything You Need$/ })).toBeVisible();

    // 6. Verify Join Team section waitlist form renders
    await expect(page.locator('h2').filter({ hasText: 'Ready to Level Up' })).toBeVisible();
    await expect(page.locator('button', { hasText: 'Request Access Slot' })).toBeVisible();
  });
