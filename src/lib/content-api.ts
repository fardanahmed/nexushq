...
function getBackendBaseUrl(): string | null {
  ...
  if (!rawUrl) {
    console.warn(
      'BACKEND_API_URL or PUBLIC_BACKEND_API_URL is not configured.'
    );
    return null;
  }
...
async function fetchFromBackend<T>(path: string, fallback: T): Promise<T> {
  ...
  try {
    ...
    if (isConnRefused || isFetchFailed) {
      ...
      console.warn(
        `[Content API] Backend offline at ${baseUrl}. Using mock fallback for ${path}`
      );
    } else {
...
export async function getFeatures(): Promise<Feature[]> {
  const result = await fetchFromBackend<{ items: Feature[] }>('/api/features', {
    items: [
      {
        id: '1',
        title: 'Smart Scheduling',
        description:
          'Automated booking system with calendar sync, timezone detection, and buffer time management.',
        icon: 'Calendar',
      ...
      },
      {
        ...
        description:
          'Track client progress, session notes, and outcomes with a centralized roster.',
        icon: 'Users',
        created_at: new Date().toISOString(),
      },
      {
        ...
        description:
          'Host, record, and manage video coaching sessions with built-in screen sharing.',
        icon: 'Video',
        created_at: new Date().toISOString(),
      },
      {
        ...
        description:
          'Accept payments, send invoices, and manage subscriptions — all in one dashboard.',
        icon: 'CreditCard',
        created_at: new Date().toISOString(),
      },
    ],
